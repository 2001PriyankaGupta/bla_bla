...
@section('content')
                ...
                <div class="settings-tab-pane" id="roles-permissions-tab">
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header d-flex justify-content-between align-items-center">
                                    <h5 class="card-title mb-0"><i class="fas fa-users me-2"></i>Admin Users Management
                                    </h5>
                                    <button type="button" class="btn btn-light btn-sm" data-bs-toggle="modal"
                                        data-bs-target="#adminUserModal">
                                            ...
                                            <tbody>
                                                @forelse($admins as $admin)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            @if ($admin->permissions)
                                                                @foreach (array_slice($admin->permissions, 0, 3) as $permission)
                                                                    <span
                                                                        class="badge bg-light text-dark small me-1">{{ $permission }}</span>
                                                                @endforeach
                                                                @if (count($admin->permissions) > 3)
                                                                    <span
                                                                        class="badge bg-secondary small">+{{ count($admin->permissions) - 3 }}
                                                                        more</span>
                                                                @endif
                                                            @else
                                                        ...
                                                        <td class="text-center">
                                                            <button class="btn btn-sm btn-outline-primary edit-admin-user"
                                                            ...
                                                            @if ($admin->id !== auth()->id())
                                                                <form
                                                                    action="{{ route('admin.settings.admin-users.delete', $admin->id) }}"
                                                                    method="POST" class="d-inline">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit"
                                                    ...
                                                    <tr>
                                                        <td colspan="5" class="text-center py-5">
                                                            <div class="text-muted">
                                                                ...
                                                                <p>Click "Add Admin User" to create your first admin user.
                                                                </p>
                                                            </div>
                                                        </td>
                                                    </tr>
                    ...
                    <div class="row mt-4">
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                <div class="card-header d-flex justify-content-between align-items-center">
                                    <h5 class="card-title mb-0"><i class="fas fa-user-shield me-2"></i>Roles Management
                                    </h5>
                                    <button type="button" class="btn btn-light btn-sm" data-bs-toggle="modal"
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                <div class="card-header">
                                    <h5 class="card-title mb-0"><i class="fas fa-list-check me-2"></i>Available
                                        Permissions</h5>
                                </div>
                                <div class="card-body">
                                    @php
                                                ...
                                                <div class="row">
                                                    @foreach ($modulePermissions as $permission)
                                                        ...
                                                        @else
                                                            ...
                                                            {{-- <p class="text-danger">Invalid permission object encountered</p> --}}
                                                        @endif
                                                    @endforeach
                                                </div>
                                            </div>
    ...
    <!-- Admin User Modal -->
    <div class="modal fade" id="adminUserModal" tabindex="-1" aria-labelledby="adminUserModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header" style="background-color: var(--primary-color); color: white;">
                    <h5 class="modal-title" id="adminUserModalTitle">Add New Admin User</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <form id="adminUserForm" action="{{ route('admin.settings.admin-users.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="admin_user_name" class="form-label fw-semibold">Name</label>
                                    <input type="text" class="form-control" id="admin_user_name" name="name"
                                        required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="admin_user_email" class="form-label fw-semibold">Email</label>
                                    <input type="email" class="form-control" id="admin_user_email" name="email"
                                        required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="admin_user_password" class="form-label fw-semibold">Password</label>
                                    <input type="password" class="form-control" id="admin_user_password"
                                        name="password">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="admin_user_password_confirmation" class="form-label fw-semibold">Confirm
                                        Password</label>
                                    <input type="password" class="form-control" id="admin_user_password_confirmation"
                        ...
                        <div class="mb-3">
                            <label for="admin_user_role" class="form-label fw-semibold">Role</label>
                            <select class="form-select" id="admin_user_role" name="role_id">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-primary" id="adminUserSubmitBtn">Save Admin User</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.edit-admin-user').click(function() {
                const adminUserId = $(this).data('id');

                fetch(`/admin/settings/admin-users/${adminUserId}/edit`)
                    .then(response => {
                        if (!response.ok) {
                            throw new Error('Network response was not ok');
                        }
                    ...
                    .then(data => {
                        $('#adminUserModalTitle').text('Edit Admin User');
                        $('#adminUserId').val(data.id);
                        $('#admin_user_name').val(data.name);
                        $('#admin_user_email').val(data.email);

                        // Set role
                        if (data.roles && data.roles.length > 0) {
                            $('#admin_user_role').val(data.roles[0].id);
                        }

                        // Set permissions
                        $('.admin-permission-checkbox').prop('checked', false);
                        if (data.permissions && data.permissions.length > 0) {
                            data.permissions.forEach(permissionSlug => {
                                $(`.admin-permission-checkbox[value="${permissionSlug}"]`).prop(
                                    'checked', true);
                            });
                        }

                        // Update form action for editing
                        $('#adminUserForm').attr('action', `/admin/settings/admin-users/${data.id}`);
                        $('#adminUserForm').append('<input type="hidden" name="_method" value="PUT">');
                        $('#adminUserSubmitBtn').text('Update Admin User');
                    ...
                    .catch(error => {
                        console.error('Error:', error);
                        Swal.fire('Error', 'Failed to load admin user data', 'error');
                    });
            });

            // Reset Admin User modal when closed
            $('#adminUserModal').on('hidden.bs.modal', function() {
                $('#adminUserForm')[0].reset();
                $('#adminUserId').val('');
                $('#adminUserModalTitle').text('Add New Admin User');
                $('#adminUserForm').attr('action', '{{ route('admin.settings.admin-users.store') }}');
                $('#adminUserForm').find('input[name="_method"]').remove();
                $('#adminUserSubmitBtn').text('Save Admin User');

                // Make password fields required for new users
                $('#admin_user_password').attr('required', true);
                $('#admin_user_password_confirmation').attr('required', true);
            });
            ...
            $('.modal').on('hidden.bs.modal', function() {
                ...
                if ($form.attr('id') === 'adminUserForm') {
                    $form.attr('action', '{{ route('admin.settings.admin-users.store') }}');
                    $form.find('input[name="_method"]').remove();
                }
            });
